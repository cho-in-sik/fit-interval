...
export const workoutStorage = {
  async saveWorkout(workout: Omit<WorkoutRecord, 'id' | 'timestamp'>): Promise<void> {
    try {
      ...
      await AsyncStorage.setItem(WORKOUT_STORAGE_KEY, JSON.stringify(limitedRecords));
    } catch (error) {
  ...
  async deleteWorkout(id: string): Promise<void> {
    try {
      ...
      await AsyncStorage.setItem(WORKOUT_STORAGE_KEY, JSON.stringify(filteredRecords));
    } catch (error) {
